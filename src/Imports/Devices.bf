...
namespace BeefBass
{
    extension Bass
    {
        ...
        /// <summary>
        /// Gets the current output master volume level... 0 (silent) to 1 (max).
        /// </summary>
        /// <remarks>
        /// <para>When using multiple devices, the current thread's device setting (as set with <see cref="CurrentDevice" />) determines which device this function call applies to.</para>
        ...
        /// <exception cref="Errors.Unknown">Some other mystery problem!</exception>
        public static double Volume => BASS_GetVolume();

		/// <summary>
		/// Sets the current output master volume level... 0 (silent) to 1 (max).
		/// </summary>
		/// <remarks>
		/// <para>When using multiple devices, the current thread's device setting (as set with <see cref="CurrentDevice" />) determines which device this function call applies to.</para>
		...
		/// <exception cref="Errors.Parameter">Invalid volume.</exception>
		/// <exception cref="Errors.Unknown">Some other mystery problem!</exception>
		public static Result<void> SetVolume(double Volume)
		{
			if (!BASS_SetVolume((float)Volume))
				return .Err;
			return .Ok;
		}
        #endregion
        ...
        /// <summary>
        /// Gets the device setting of the current thread... 0 = no sound, 1 = first real output device.
        /// </summary>
        /// <remarks>
        /// <para>A return value of -1 indicates error. Use <see cref="LastError" /> to get the error code. Throws <see cref="BassException"/> on Error while setting value.</para>
        /// <para>
        ...
        /// </para>
        /// </remarks>
        public static int32 CurrentDevice => BASS_GetDevice();

		/// <summary>
		/// Sets the device setting of the current thread... 0 = no sound, 1 = first real output device.
		/// </summary>
		/// <remarks>
		/// <para>
		/// Simultaneously using multiple devices is supported in the BASS API via a context switching system -
		...
		/// </para>
		/// </remarks>
		public static Result<void> SetCurrentDevice(int32 Device)
		{
        ...
        /// </remarks>
        /// <exception cref="Errors.Device">The device number specified is invalid.</exception>
        public static Result<DeviceInfo> GetDeviceInfo(int32 Device)
        {
            if (!GetDeviceInfo(Device, let info))
                return .Err;
            return info;
        }
        #endregion
        ...
        /// When using multiple devices, the current thread's device setting (as set with <see cref="CurrentDevice"/>) determines which device this function call applies to.
        /// </remarks>
        public static Result<BassInfo> Info
        {
            get
            {
                if (!GetInfo(let info))
