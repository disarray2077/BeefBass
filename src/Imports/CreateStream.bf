...
namespace BeefBass
{
    extension Bass
    {
        ...
        public static int32 CreateStream(StringView File, int64 Offset = 0, int64 Length = 0, BassFlags Flags = BassFlags.Default)
        {
            return BASS_StreamCreateFile(false, File.ToScopedNativeWChar!(), Offset, Length, Flags | BassFlags.Unicode);
        }
        ...
        /// <exception cref="Errors.No3D">Could not initialize 3D support.</exception>
        /// <exception cref="Errors.Unknown">Some other mystery problem!</exception>
        public static int32 CreateStream(void* Memory, int Offset, int64 Length, BassFlags Flags = BassFlags.Default)
        {
            return BASS_StreamCreateFile(true, (void*)((int)Memory + Offset), 0, Length, Flags);
        }
        
        /// <summary>
        /// Creates a sample stream from an MP3, MP2, MP1, OGG, WAV, AIFF or plugin supported file in memory (uint8[]).
        /// </summary>
        /// <param name="Memory">A uint8[] containing file data.</param>
        /// <param name="Offset">Offset to begin streaming from.</param>
        /// <param name="Length">Data length (needs to be set to the length of the memory stream in bytes which should be played).</param>
        /// <param name="Flags">Any combination of <see cref="BassFlags"/>.</param>
        /// <returns>If successful, the new stream's handle is returned, else 0 is returned. Use <see cref="LastError" /> to get the error code.</returns>
        ...
        /// <exception cref="Errors.No3D">Could not initialize 3D support.</exception>
        /// <exception cref="Errors.Unknown">Some other mystery problem!</exception>
        public static int32 CreateStream(uint8[] Memory, int Offset, int64 Length, BassFlags Flags)
        {
            return CreateStream(Memory.Ptr, Offset, Length, Flags);
        }
        ...
        /// <exception cref="Errors.No3D">Could not initialize 3D support.</exception>
        /// <exception cref="Errors.Unknown">Some other mystery problem!</exception>
        public static int32 CreateStream(StringView Url, int32 Offset, BassFlags Flags, DownloadProcedure Procedure, void* User = null)
        {
            return BASS_StreamCreateURL(Url.ToScopedNativeWChar!(), Offset, Flags | BassFlags.Unicode, Procedure, User);
        }
    }
}